...
/** Crowdin's export stops at the closing brace, which git then flags as a missing newline. */
function endWithNewline(locale) {
    const file = `${TRANSLATIONS}/${locale}/website.json`;
...
for (const locale of downloaded) {
    ...
    // The download runs with `skip_untranslated_strings` and without `export_only_approved`,
    // so a key being present in the file means somebody translated it. Approval is a separate
    // state in Crowdin and deliberately does not count towards a language's progress here.
